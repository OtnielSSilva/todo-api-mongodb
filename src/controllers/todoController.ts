...
export default {
	async getTodos(req: Request, res: Response) {
		try {
			...
			return res
				.status(400)
				.json({ status: 400, msg: { error: "Erro ao listar tarefas" } });
		}
	},
	...
	async createTodo(req: Request, res: Response) {
		...
		try {
			...
			return res
				.status(400)
				.json({ status: 400, msg: { error: "Erro ao criar tarefa" } });
		}
	},
	...
	async deleteTodo(req: Request, res: Response) {
		...
		try {
			...
			return res
				.status(204)
				.json({ status: 204, msg: "Tarefa excluida com sucesso" });
		} catch (error) {
			return res
				.status(400)
				.json({ status: 400, msg: { error: "Erro ao excluir tarefa" } });
		}
	},
};
